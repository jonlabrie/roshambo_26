...
- **World Record hologram**: the cross-platform global champion. A Roblox (or linked) champion's holo shows their real avatar; an unlinked web champion appears as a filtered-name "outsider" glow — no dummy avatars. **All externally-sourced display names must pass `TextService` filtering before display** (Roblox text-safety requirement). Filtering can throttle, error, hang, or redact to hashes, so it is **never in the reveal critical path**: holo name updates run asynchronously off `/leaderboards` polling, with the filter call `pcall`-wrapped; the holo retains its last-known-good name until a new result resolves. On failure or heavy redaction, fall back to a generic epithet ("a challenger from the wider world") rather than rendering raw text or `####`; cache successful filter results per name. The same World Record motif later appears in the PWA.
