...
1. **Atlas database user:** reuse the existing **`roshambo_app`** user — no new user needed. If its role is cluster-wide (`readWriteAnyDatabase`/`atlasAdmin`), it already covers `roshambo-dev` (created implicitly on first write); if it's scoped to `roshambo` only, add `readWrite @ roshambo-dev`. (The URI currently in `server/.env` uses a *different*, non-working user `jonlabrie_db_user` — that is the `bad auth`; switching to the `roshambo_app` string fixes it.)
