...
| Throw drum | Hexagonal, horizontal axis, water-driven escapement (always dwells on a full face); holds prior throw, drumrolls in TALLY, locks new throw on the bell strike; two-sided via 180° face pairing. Built as a true hex prism: central bearing shaft + 12 spokes (6/end) to the corners + 6 flush rim faces (no core/housing); each slab oriented Front-outward so the glyph SurfaceGui sits on Front | Turns like the waterwheel (water kinship); prior-throw reminder at lockout; mechanically married to the striker; open spoked structure reads as a real drum (user concept + gate refinements) |
| Drum caret/caron asymmetry | The ∧ (caret) is the only non-vertically-symmetric glyph, so the drum's two windows necessarily read it flipped: one side sees ∨ (caron = the mascot's smile = the **"happy side"** of the arena), the other sees ∧ (caret = **"sad/angry side"**). ○ and ─ read identical both sides. **Intentional — do NOT force both sides upright.** | Embrace the brand's happy-face logic physically; fuels happy/sad-side framing in lore + virtual merch (user direction, 2026-06-08) |
