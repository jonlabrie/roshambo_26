...
| §7 tatami watermark | **Deferred** out of 4b (no home without tatami). Brand presence in 4b = bell-boss mascot glyphs + hero tile idle face | User choice over gravel-rake or paver alternatives |
...
- **Bowl**: three terrace tiers (cushions, tea tables, lanterns) with amphitheater sightlines to bell + jumbotron.
...
- **Jumbotron**: floating above the basin per parent §4; 4b builds its lacquered frame, tile faces, and the true-3D hero tile.
...
| 2 — The monument | Bell, pavilion, wheel, flume, basin + ripple system (fallback rings first), draw chain visually hooked to existing `HammerCurve` cues | Full round reads at the center |
| 3 — The bowl | Terrace materials, teahouses + interiors, lantern rig | Bowl feels inhabitable |
| 4 — Periphery & inhabitants | Torii, koi pond, foliage, fate models wired through manifest, champions (`ChampionSeats` + statues + stele) | Fates read in real dress; champions dress from live data |
...
- The full round reads in the real set: draw → latch click → tremble → release → BONG → ripples → reveal, under final lighting.
- Manifest validation green with all-real asset entries; all Lune tests green (113 existing + `ChampionSeats`).
