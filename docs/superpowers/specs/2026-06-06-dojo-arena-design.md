...
**Chase rules — fate is patient, not punctual.** There is **no guaranteed catch**: contact ends the flight, and skilled players can survive comically long — that survival IS the show. Intensity escalates on a stage curve (more entities, faster spawns, tighter targeting) toward near-impossibility, but **plateaus at a per-victim entity cap** so cost stays bounded while the antics continue. Standing still = caught in ~1.5s (acceptance is faster — fittingly zen). Dooms pursue through doors, over bridges, into the koi pond (soggy paper storm slaps — bonus gag). The pick UI stays live mid-flight.

**Cross-round rules**: a flight may continue indefinitely across rounds. If the fleeing player **loses again**, the active fate catches instantly and the new fate begins **one intensity stage higher** (fates compound). If they **WIN while fleeing**, the gold-petal burst **banishes the doom** — the redemption arc the crowd watches a long flight hoping for. SAFE changes nothing; the flight continues.
...
**Event architecture**: M3's `RevealResult` fires per-player with only that player's outcome — insufficient for arena-wide theater. M4 adds a **`RevealTheater` broadcast** (FireAllClients) carrying `{worldThrow, distribution, results: {userId → result}}` so every client renders all grows/fates/umbrellas locally. Pursuit simulation is client-side with **per-victim authority**: the doomed player's own client simulates their pursuit and detects contact, firing a small **`FateResolved` remote** (victim → server → broadcast) so all clients land the catch at the same moment; other clients render approximations and snap on that event. No server physics; traffic is one tiny event per catch. The per-player `RevealResult` remains for authoritative personal UI. No new REST surface; no new server→Node traffic.
...
| `DoomEscalation` | Pure pursuit math: intensity stage as f(elapsed); spawn schedules per fate type; entity-cap plateaus; homing/ambush position steps; stand-still fast-catch threshold; compound-stage and win-banish rules |
...
| LOSS effect | Three Fates: throw-matched escalating pursuits, NO guaranteed catch — contact ends it; win banishes, repeat loss compounds | "Destroyed by what beat you" made flesh; skilled flights are spectator theater; dwell time; pick-while-fleeing |
