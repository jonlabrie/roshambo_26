...
- **Pad classes.** Two classes with class-specific structural vocabularies:
  ...
  - **Second-story eligibility is sightline-gated.** A purchasable second story on a *lower, foreground, same-wall* perch can rise into a *higher, behind-and-above* neighbor's down-valley view (the prized sight is the water coursing down the valley floor, not the horizon; cross-canyon perches never block — they're the scenery). So second-story eligibility per site is gated by a **same-wall foreground sightline test** (audit tool, 2026-07-04): a perch that would obstruct a neighbor is capped at single-story ("the low perch" — a tier/marketing distinction), or its victim is nudged up a few studs. As-audited near-wall conflicts: TH07→TH05 and Proto→TH01 (both 2nd-story-induced, tiny); TH12→TH11 (far wall); plus a pre-existing single-story block TH04→TH06 unrelated to tiers. The audit needs the watercourse centerline to resolve long-range cases exactly.
