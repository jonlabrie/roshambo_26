...
- **Pad classes.** Two classes with class-specific structural vocabularies:
  ...
  - **Vacant pad states (per class).** Cliff perches stand as **dark teahouses** — base form, unlit shoji — so the gorge vista reads fully built at any occupancy, and **shoji glow becomes the occupancy signal** (dark paper = claimable, warm glow = someone's home), legible across the canyon at dusk with zero UI. Claiming dissolves the dark shell and builds the claimant's own tier in its place, preserving the construction flourish. Valley-floor pads sit as **pocket gardens** (raked gravel, set stones, one stone lantern) that persist as the yard when the teahouse materializes over them — a mostly-vacant residential valley reads as a street of gardens, not empty lots. Shared gate signaling: unlit chōchin on the claim post when vacant, lit when claimed, owner's noren/mon at the entrance.
  - **Second-story view obstruction is a light-touch consideration, not a hard gate.** A second story on a *lower, foreground, same-wall* perch can rise into a *higher, behind-and-above* neighbor's down-valley view (the prized sight is the water coursing the valley floor, not the horizon; cross-canyon perches never block — they're scenery). But **transitory pad assignment is the real mitigator**: players don't own a fixed pad, so no one is permanently stuck behind a neighbor's upgrade, and a marginal conflict self-heals next session. So we do NOT hard-cap sites; we just avoid egregious cases. A same-wall foreground sightline **audit tool** exists (2026-07-04) but its purely-geometric metric over-reports — it ignores terrain occlusion (e.g. it flagged TH04→TH06, which is actually blocked by the spur the tunnel runs through) and doesn't know the watercourse position; treat its output as a hint, not truth. Reviewed conflicts were all minor (TH07→TH05, Proto→TH01, TH12→TH11); the only one worth a cheap pre-empt is **lowering the TeahousePrototype** slightly.
  - **Design for pad access, not teahouse access.** Access structures (spurs, stairs, bridges) terminate at the **pad boundary** — the future claim-gate post — *outside the site's maximum expansion envelope*, never at the current teahouse's deck/engawa. The teahouse is data that changes shape with tiers; anything registered to its current form breaks or blocks on upgrade (cautionary example: `Teahouse3Stair` sits inside 03's front envelope and zeroed its audit grade). Envelope table: `references/2026-07-04-teahouse-envelope-audit.md`.
