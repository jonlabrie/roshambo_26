...
The user's canyon compass, mapped to world axes (confirmed 2026-06-27):
...
| **West** | **−Z** | the cliff wall (rises to ~160–173); deck tucks against it |
| **North** | **−X** | up-canyon; the upper path enters here AND the trail exits toward FW10 here |
...
- **Onward heading** FW11→FW10: `(−0.92, 0, 0.39)` — i.e. −X and +Z. Both the entry and the exit lie on the
  **North (−X)** side; the deck extends South/East to give turning room. Net turn ≈ 180°.
- **Natural ledge:** a narrow band at grade **~138** running x≈120–140 in the z≈−74 to −78 strip, bounded by
  the cliff wall to the West (z≈−82 → 160+) and the drop to the East (z≥−70 falls toward the basin ~122–125).
...
- **Posts:** `Material.Wood`, **1.5 × 1.5** square, from girders down to terrain: short (~2–4) on the
  West/cliff edge, tall (~12–16) on the East/downhill edge (ground ~122–125 there). One simple **X-brace**
...
- **Open edges (no railing):** **West** (against the cliff) and **North** (path enters + steps leave).
...
1. **Enter (North/−X edge):** the last cobble steps of the upper path land flush onto the deck at grade ~138.
...
3. **Leave (North/−X edge, offset +Z):** **2–3 wooden step-downs** (deck timber) off the edge shed the first
