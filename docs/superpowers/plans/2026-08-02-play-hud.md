...
**Goal:** Replace the provisional Roblox play UI with a minimal hangout HUD, a maximal "ledger" takeover mode, and four onboarding beats — and give unbanked points a persistent, always-collectable representation.

> **Mid-plan reversal.** Tasks 3, 4 and 9 build a *blocking win gate* (a WIN prevents throwing until the player answers RISK or BANK). **Task 11 withdraws it** on the owner's ruling: "RISK IT" is wager language on a mechanic that is deliberately not a wager, and Roblox proscribes simulated gambling in a kid-first experience. It is replaced by a pot indicator with BANK THESE; throwing again is riding. Tasks 3/4/9 below read as executed — this plan is a history, not only a target — but **the shipped design is Task 11's**.

**Architecture:** Pure, Lune-tested view-model modules in `roblox/src/shared` (`HudModel`, `LedgerModel`, `OnboardingBeats`) rendered by thin controllers in `roblox/src/client`, following the split `TeahouseController`/`TeahouseMenuModel` already proves. `main.client.luau` is reduced to remote wiring. The fate gate stays server-authoritative in `FateRegistry`; the win gate that Tasks 3/4/9 add is removed again by Task 11 (see the reversal note above).
