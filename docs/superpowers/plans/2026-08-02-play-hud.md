...
- **Tests live beside their subject and most already exist. Extend, never overwrite.**
  Server: `src/wallet.test.ts`, `src/models/models.test.ts`, `src/engine/Settlement.test.ts`,
  `src/routes/apiV1.test.ts` — there is no `__tests__/` directory; `vitest.config.ts` globs
  `src/**/*.test.ts`. Server tests run against a real in-memory Mongo via
  `connectTestDb` / `clearTestDb` / `disconnectTestDb` from `./test/db`, with `User.create(...)`
  for fixtures — follow the surrounding file's style rather than the sketch in this plan.
  Roblox: `roblox/tests/*.spec.luau`. Read the target file before writing; Task 1 found
  `Glyphs.spec.luau` already present with broader coverage than this plan assumed.
...
**Latch the pair — one answer per binding.** The first tap on *either* button must disable *both*
and stay disabled until the echo arrives:

```luau
-- RISK and BANK are guarded by different fields server-side (RISK on unresolvedWin, BANK on the
-- observed pointsAtStake), so two conflicting requests for the same win can both pass their own
-- filter: RISK clears the gate, then a BANK already in flight banks the pot anyway, silently
-- turning "keep riding" into a bank. Same-choice double-taps ARE safe; it is the cross-choice
-- race that is not. One latch here removes the whole class.
local answering = false
```

Clear `answering` when the `ProfileUpdate` echo shows `unresolvedWin == false`, or on the next
round, whichever lands first — never leave it latched on a dropped response.

(Found by the Task 4 review, which traced it in `wallet.ts:97-114` and correctly rated it Minor
because it needs two conflicting requests, not a double-tap. Fixing it here is cheaper and more
certain than a second server-side guard.)
