...
- Produces: `NetworkClient:postResolveWin(robloxUserId: string, choice: string)`.
...
buttons go to `filter`-equivalent dimming — reduce `BackgroundTransparency`/`ImageTransparency`
and set `AutoButtonColor = false`.
...
One row, one switch — *Escalation prompts* — firing `EventBus.HudPreference`. Wire it in
`main.client.luau` to a new `SetHudPreference` remote, or extend an existing preference remote if
one fits. Size the row for later additions.
...
**Interfaces:**
- Consumes: `OnboardingBeats`, `EventBus.Onboard` fired with an event name.
- Produces: a `seenBeat` write through the preferences route.
...
mark the beat seen locally **and** fire the server write — an optimistic local mark keeps it from
re-firing before the round trip lands.
