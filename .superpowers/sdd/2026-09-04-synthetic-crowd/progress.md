...
Post-handoff 2026-09-04: readability experiment DEFECT — all six humans voted in one tally (cross-voting); fixed to one runSimulation per human (same seed, transitions from the random-only run), test added. Honest table at the settled mix: counter 44.0–44.5% alone, under the 45% floor. Re-tuned DEFAULT_MIX conform 30→35, random 20→15 (one bot of 30). Seeds 1–5 alone: counter 49.9–50.8, second 37.3–38.0, random 29.6–30.0, oracle 57.2–57.9. Task 11's direction ruling reversed for the reversed failure; constraints kept. sim-*.txt regenerated from the new default. Dev needs a source redeploy to pick it up.
